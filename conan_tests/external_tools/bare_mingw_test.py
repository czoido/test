...
class BuildMingwTest(unittest.TestCase):
    ...
    def build_mingw_test(self):
        ...
        print(os.environ)
        print(sys.path)
        cmd = 'dir {}'.format(os.environ["PYTHONPATH"])
        out = os.popen(cmd)
        print(cmd)
        print(out.read())
        cmd = 'dir {}'.format(os.path.join(os.environ["PYTHONPATH"],"conans"))
        out = os.popen()
        print(cmd)
        print(out.read())
        cmd = 'dir {}'.format(os.path.join(os.environ["PYTHONPATH"],"conans","test"))
        out = os.popen(cmd)
        print(cmd)
        print(out.read())
        cmd = 'dir {}'.format(os.path.join(os.environ["PYTHONPATH"],"conans","test","integration"))
        out = os.popen(cmd)
        print(cmd)
        print(out.read())
        with tools.remove_from_path("bash.exe"):
